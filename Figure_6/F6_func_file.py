...
def Apply_correction(ds_image_, damn_image_):
    target_size = 240
    out_size = 300
    remap_offset = 30
    drift = 25
    
    corners = np.array([[0,0], [0,239], [239,0+drift], [239,239+drift]])
    homography = Get_correction(corners, target_size, remap_offset)
    
    corrected_ds_image = cv2.warpPerspective(ds_image_, homography, (out_size, out_size))
    corrected_damn_image = cv2.warpPerspective(damn_image_, homography, (out_size, out_size))
    
    return corrected_ds_image[50:-50,30:-70], corrected_damn_image[50:-50,30:-70]
