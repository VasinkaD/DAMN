...
def Reconstruct_data_with_model(data_, model_, batches=10):
    ...
    for i in range(int(data_.shape[0] / batches)):
        predicted[i*batches:(i+1)*batches,:,:] = np.squeeze(model_.predict(data_in_renormed[i*batches:(i+1)*batches,:,:,None]))
